...
export class StoreComponent {
  ...
  readonly tags$: Observable<TagStoreModel[]> = this._storesService.getAllTags();

  readonly storeList$: Observable<StoreQueryModel> = combineLatest([
    this.storeId$,
    this.tags$
  ]).pipe(
    switchMap(([store, tags]) => this._storesService.getOne(store).pipe(
      map((store) => this._mapToStoreQuery(tags, store))
    ))
  )
  ...
  constructor(private _activatedRoute: ActivatedRoute, private _storesService: StoresService, private _productsService: ProductsService) {
  }


  private _mapToStoreQuery(Tag: TagStoreModel[], store: StoreModel): StoreQueryModel {
    const storeTagMap = Tag.reduce((a, c) => ({ ...a, [c.id]: c }),
